...
def get_system_status(hostname):
        ...
        url = f"https://{CONFIG_DATA['{hostname}'}]}:6443/sma/api/v2.0/config/centralizedServices/systemStatus"
        print(url)
        ...
        try:
            ...
            # response = requests.get(url, auth=HTTPBasicAuth(CONFIG_DATA["SMA_USERNAME"], CONFIG_DATA["SMA_PASSWORD"]), verify=False).text.encode('utf8').decode('ascii', 'ignore')
            ...
            if response.status_code >= 200 and response.status_code < 300:
                ...
                for dati in response.json()["data"]:
                    #print(message)
                    #message = message.text.encode('utf8').decode('ascii', 'ignore')
                    #message = message.['text'].encode('utf8').decode('ascii', 'ignore')
                    #print(message)
                    return_data.append(dati)
